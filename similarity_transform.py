...
def absolute_orientation(from_points, to_points, targets):
...
if __name__ == "__main__":
    ...
    #                         [2, 5, 8],
    #                         [3, 1, 4]])
    from_points = np.random.randint(low=0, high=10,size=(5,3))
    ...
    to_points = np.random.randint(low=0, high=10,size=(5,3))
    targets = np.random.randint(low=0, high=10,size=(2,3))
    
    rslt = similarity_transform(from_points, to_points, targets)
    assert np.allclose(to_points, rslt), "test for 3D space fail"
